...
import { buildFromMatchup } from './scenario-builder.js';
import { runBatch } from '../engine/sequence.js';
import { createDungeonAI } from '../ai/dungeon-ai.js';
import { createVisitorAI } from '../ai/visitor-ai.js';
import { buildDungeonProfile, buildVisitorProfile, buildCreatureProfile } from './profile-builder.js'; // ═══════════════════════════════════════════════════════════════
// MATRIX EXECUTION
// ═══════════════════════════════════════════════════════════════
...
function runMatrix(matchups, options = {}) {
  ...
  for (let i = 0; i < matchups.length; i++) {
    ...
    try {
      ...
      // Create AI instances
      // Build inferred profiles from deck composition
      const dungeonProfile = buildDungeonProfile(matchup.encounters, matchup.dungeonProfile);
      const dungeonAI = createDungeonAI(dungeonProfile);

      let visitorAI;
      if (matchup.members) {
        const visitorProfile = buildVisitorProfile(matchup.members, matchup.visitorProfile);
        visitorAI = createVisitorAI(visitorProfile);
      } else {
        const creatureProfile = buildCreatureProfile(matchup.creature);
        visitorAI = createVisitorAI(creatureProfile);
}
...
export { runMatrix, flattenResult, flattenAll };